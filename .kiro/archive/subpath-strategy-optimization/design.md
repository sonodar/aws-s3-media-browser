...
#### TanStack Query DevTools（キャッシュ状態確認）

5. **キャッシュキーの確認**
   - 画面右下の TanStack Query DevTools アイコンをクリック
   - `storageItems` で始まるクエリキーを確認
   - キーの形式: `["storageItems", identityId, path]`
     - 例: `["storageItems", "ap-northeast-1:xxx", "photos"]`

6. **キャッシュヒットの確認**
   - フォルダを移動すると新しいクエリが `fresh` 状態で追加される
   - 既に訪問済みのフォルダに戻ると `stale` 状態のキャッシュを使用（API 呼び出しなし）
   - クエリを選択して `Data` タブでキャッシュされた内容を確認可能

7. **Mutation 後のキャッシュ無効化確認**
   - フォルダ操作（削除/移動/リネーム）を実行
   - DevTools で該当パスのクエリが `stale` (灰色) になることを確認
   - **重要**: 配下パスのキャッシュも一緒に無効化されることを確認
     - 例: `photos/vacation` を閲覧後、`photos` フォルダを削除
     - `["storageItems", id, "photos"]` と `["storageItems", id, "photos/vacation"]` の両方が `stale` になる

### Mutation 後のキャッシュ無効化（動作確認）
...
10. **ファイル/フォルダ削除**
    - アイテムを削除 → 一覧から即座に消える
    - TanStack Query DevTools で該当パスのキャッシュが無効化されていることを確認

11. **ファイル/フォルダ移動**
    - アイテムを別フォルダに移動 → 元フォルダから消え、移動先に表示される
    - TanStack Query DevTools で移動元・移動先の両方のキャッシュが無効化されていることを確認

12. **リネーム**
    - アイテムをリネーム → 一覧に新しい名前で表示される
    - フォルダリネームの場合、旧パス配下のキャッシュも無効化されていることを確認
