...
7. The メディアブラウザ shall セルフサインアップ機能を無効化し、管理者のみがユーザーを作成できるようにする
8. The メディアブラウザ shall サインアップフォームを非表示にし、サインインフォームのみを表示する

### Requirement 2: ファイルブラウジング機能

**Objective:** As a ユーザー, I want S3 バケット内の自分専用のファイルとフォルダを閲覧したい, so that 保存されているメディアを確認・探索できる

#### Acceptance Criteria

1. When ユーザーが認証済みでメインページにアクセスした場合, the メディアブラウザ shall ユーザー専用のストレージディレクトリ（`media/{entity_id}/`）内容を一覧表示する
...
### Requirement 6: ユーザー分離

**Objective:** As a ユーザー, I want 自分のファイルだけにアクセスしたい, so that 他のユーザーのファイルと混同しない

#### Acceptance Criteria

1. When ユーザーがファイルをアップロードした場合, the メディアブラウザ shall ユーザー専用のパス（`media/{entity_id}/`）にファイルを保存する
2. While ユーザーがファイル一覧を表示している場合, the メディアブラウザ shall 自分のファイルのみを表示し、他のユーザーのファイルは表示しない
3. The メディアブラウザ shall Cognito Identity ID を使用してユーザーを識別し、ストレージパスを自動的に分離する
